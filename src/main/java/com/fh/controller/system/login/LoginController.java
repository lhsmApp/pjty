...
public class LoginController extends BaseController {
	...
	public ModelAndView toLogin() throws Exception {
		...
		/*********跳过登录页面直接显示Guest主页******************/
		pd.put("KEYDATA", "Guest,fh,123456");
		Map<String, String> map=(Map<String, String>) loginGuestExe(pd);
	...
	public Object loginGuest() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		return loginGuestExe(pd);
	}
	
	private Object loginGuestExe(PageData pd) throws Exception{
		Map<String, String> map = new HashMap<String, String>();
		String errInfo = "";
		...
		if (null != KEYDATA && KEYDATA.length == 2) {
			Session session = Jurisdiction.getSession();
			// String sessionCode =
			// (String)session.getAttribute(Const.SESSION_SECURITY_CODE);
			// //获取session中的验证码
			// String code = KEYDATA[2];
			// if(null == code || "".equals(code)){//判断效验码
			// errInfo = "nullcode"; //效验码为空
